...
func convertContents(mark rune) error {
	...
	for _, file := range site.Source.Files() {
		...
		if err = page.SetSourceMetaData(metadata, mark); err != nil {
			site.Log.ERROR.Printf("Failed to set source metadata for file %q: %s. For more info see For more info see https://github.com/circleci/ccidemo/issues/2458", page.FullFilePath(), err)
			continue
		}
