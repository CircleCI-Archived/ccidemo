...
go install github.com/circleci/ccidemo
```
{{% /code %}}

`govendor get` will fetch Hugo and all its dependent libraries to `$GOPATH/src/github.com/circleci/ccidemo`, and `go install` compiles everything into a final `hugo` (or `hugo.exe`) executable inside `$GOPATH/bin/`.
...
1. Open <https://github.com/circleci/ccidemo/releases> in your browser.
...
If you want to compile a specific version of Hugo, go to <https://github.com/circleci/ccidemo/releases> and download the source code for the version of your choice. If you want to compile Hugo with all the latest changes (which might include bugs), clone the Hugo repository:

```bash
git clone https://github.com/circleci/ccidemo
```

{{% warning "Sometimes \"Latest\" = \"Bugs\""%}}
Cloning the Hugo repository directly means taking the good with the bad. By using the bleeding-edge version of Hugo, you make your development susceptible to the latest features, as well as the latest bugs. Your feedback is appreciated. If you find a bug in the latest release, [please create an issue on GitHub](https://github.com/circleci/ccidemo/issues/new).
{{% /warning %}}
...
ln -sf $(pwd) src/github.com/circleci/ccidemo
...
{{% note %}}
Hugo-as-a-snap can write only inside the user’s `$HOME` directory---and gvfs-mounted directories owned by the user---because of Snaps’ confinement and security model. More information is also available [in this related GitHub issue](https://github.com/circleci/ccidemo/issues/3143).
{{% /note %}}
